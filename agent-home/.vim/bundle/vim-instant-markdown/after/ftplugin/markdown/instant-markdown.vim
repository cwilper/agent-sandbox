...
function! s:refreshView()
    ...
    call s:system("curl -X PUT -T - http://localhost:18090/ &>/dev/null &",
                \ s:bufGetContents(bufnr))
endfu

function! s:startDaemon(initialMD)
    call s:system("instant-markdown-d --port 18090 &>/dev/null &", a:initialMD)
endfu
...
function! s:killDaemon()
    call system("curl -s -X DELETE http://localhost:18090/ &>/dev/null &")
endfu
